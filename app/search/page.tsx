...
export default async function SearchPage({ searchParams }: SearchPageProps) {
    ...
    let offers: { id: string; product_name: string; price: string; original_price?: string; image_url: string | null; market_id: string; markets: { name: string; city: string } | null }[] = [];
    ...
    // 3. Offers/product search
    if (type === 'offers' && q) {
        const { data } = await supabase
            .from('offers')
            .select('id, product_name, price, original_price, image_url, market_id, markets(name, city)')
    ...
    const hasResults = exactPlzMarkets.length > 0 || otherCityMarkets.length > 0 || nameMatchMarkets.length > 0 || offers.length > 0;
    const totalResults = exactPlzMarkets.length + otherCityMarkets.length + nameMatchMarkets.length + offers.length;
                            ...
                            {hasResults && (
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        ...
                                        {(nameMatchMarkets.length > 0 || offers.length > 0) && (
                                            <div className="text-center">
                                                <div className="text-3xl font-black text-white" style={{ fontFamily: 'var(--font-playfair)' }}>
                                                    {nameMatchMarkets.length + offers.length}
                                                </div>
                                                <div className="text-xs text-white/60 font-medium">Treffer</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                    ...
                    <div className="text-center py-16 sm:py-24">
                        ...
                        >
                            ...
                            <div
                                ...
                                style={{ background: 'var(--terracotta)', animationDuration: '2.5s', animationDelay: '0.5s' }}
                            />
                            {/* Main icon container */}
                            <div
                                ...
                                style={{
                                    background: 'white',
                                    boxShadow: '0 20px 60px rgba(230, 168, 69, 0.3)'
                                }}
                            >
                                <svg
                                    className="w-14 h-14"
                        ...
                        >
                            {type === 'markets' ? 'Keinen Markt gefunden' : 'Keine Ergebnisse gefunden'}
                        </h2>
                        ...
                        {type === 'markets' ? (
                            ...
                            <p
                                ...
                                style={{ color: 'var(--warm-gray)' }}
                            >
                                Wir konnten leider keine {type === 'offers' ? 'Angebote' : 'Märkte'}{searchDescription ? ` für "${searchDescription}"` : ''} finden.
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <h2
                                                ...
                                                style={{ fontFamily: 'var(--font-playfair)', color: 'var(--charcoal)' }}
                                            >
                                                Angebote für &ldquo;{q}&rdquo;
                                            </h2>
                                            <p className="text-sm" style={{ color: 'var(--warm-gray)' }}>
                                                Aktuelle Deals die zu dir passen
                                            </p>
                                        </div>
                                    </div>
                                    <span
                                    ...
                                    {offers.map((offer, idx) => {
                                        ...
                                        return (
                                            <Link
                                                ...
                                                key={offer.id}
                                                className="group relative rounded-3xl overflow-hidden cursor-pointer block transition-all duration-500 hover:scale-[1.02] hover:-translate-y-1"
                                                style={{
                                                    background: 'white',
                                            ...
                                            >
                                                ...
                                                <div className="p-6">
                                                    <h3
                                                        className="font-bold text-xl mb-2 group-hover:text-[var(--terracotta)] transition-colors duration-300"
                                                        style={{ fontFamily: 'var(--font-playfair)', color: 'var(--charcoal)' }}
                                                    >
                                                        {offer.product_name}
                                                    </h3>
