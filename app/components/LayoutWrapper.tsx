...
export default function LayoutWrapper({ children, locale }: LayoutWrapperProps) {
    ...
    const handleNewsletterSubmit = async (e: React.FormEvent) => {
        ...
        if (!newsletterEmail.trim()) {
            ...
            setNewsletterMessage(t('newsletter.emailRequired'));
            return;
        }
        ...
        try {
            ...
            setNewsletterStatus('error');
            setNewsletterMessage(t('newsletter.errorRetry'));
        }
    };

    const navLinks = [
        { href: '/', label: t('nav.home') },
        { href: '/shops', label: t('nav.allMarkets') },
        { href: '/offers', label: t('nav.offers') },
        { href: '/blog', label: t('nav.magazine') },
        { href: '/for-shops', label: t('nav.forShops') },
    ];
                    ...
                    <footer className="relative overflow-hidden border-t" style={{ background: 'var(--gradient-night)', borderColor: 'rgba(255, 255, 255, 0.1)' }}>
                        ...
                        <div className="relative mx-auto max-w-7xl px-4 pt-16 pb-8 sm:px-6 lg:px-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
                                ...
                                {/* Quick Links */}
                                <div>
                                    <h4 className="font-bold mb-5 text-base text-white">{t('footer.discover')}</h4>
                                    <ul className="space-y-3">
                                        {[
                                            { label: t('footer.allMarkets'), href: '/shops' },
                                            { label: t('footer.currentOffers'), href: '/offers' },
                                            { label: t('footer.magazine'), href: '/blog' },
                                            { label: t('footer.newShops'), href: '/shops/new' },
                                            { label: t('footer.myFavorites'), href: '/favorites' },
                                        ].map((item, idx) => (
                                            <li key={idx}>
                                ...
                                {/* For Shops */}
                                <div>
                                    <h4 className="font-bold mb-5 text-base text-white">{t('footer.forShops')}</h4>
                                    <ul className="space-y-3">
                                        {[{ label: t('footer.registerShop'), href: '/for-shops' }].map((item, idx) => (
                                            <li key={idx}>
                                                <Link href={item.href} className="text-white/70 hover:text-white transition-colors text-sm flex items-center gap-2 group">
                                                    <svg className="w-3 h-3 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                                ...
                                {/* Newsletter */}
                                <div>
                                    <h4 className="font-bold mb-5 text-base text-white">{t('footer.newsletter')}</h4>
                            ...
                            <div className="pt-8 border-t flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
                                <p className="text-sm text-white/50">
                                    © {new Date().getFullYear()} {t('common.brandName')}. {t('common.madeWithLove')}
                                </p>
